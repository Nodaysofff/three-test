...
el.innerHTML = "<div class='first_loading_animation' style='text-align:center; width:100%;'><img src='" + app_url + "images/ripple.svg' class='loading-img' style='width:auto !important; float:none !important; text-align:center !important; border:none !important;' /></div>";
...
function scriptLoadHandler() {
        ...
        loadScript(app_url + "libs/js/website_test_fix.js?v=2", function(){
            loadScript(app_url + "libs/js/masonry/masonry4.2.2.min.js", function(){
                loadScript(app_url + "libs/js/swiper/swiper.min.js", function(){
                    loadScript(app_url + "libs/js/moment2.30.1.min.js", function(){
    ...
    $grid = new Masonry('.sk-ig-all-posts', {
         itemSelector: '.sk-instagram-feed-item',
         columnWidth: '.sk-instagram-feed-item-sizer',
         percentPosition: true,
         transitionDuration: 0
...
function observeMasonryItems(sk_instagram_feed) {
    const wrapper = sk_instagram_feed.find
        ? sk_instagram_feed.find('.sk-ig-all-posts')[0]
        : sk_instagram_feed.querySelector('.sk-ig-all-posts');
    if (!wrapper) return;
    ...
    const debounceRealign = () => {
        ...
        resizeTimeout = setTimeout(() => {
            if (wrapper.isConnected) applyMasonry();
        }, 100);
    };
    ...
    const observeItems = () => {
        ...
        wrapper
            .querySelectorAll('.sk-instagram-feed-item')
...
function openLinkTab(){
    var open_link = jQuery('.sk-instagram-feed').find('.open_link_in_new_tab').text() == 1 ? "_blank" : "_parent";
    return open_link;
}
    ...
    if(feed_item_container.find('.swiper-container').length == 0){
        ...
        if(data_type == 'video' && video_url && !video_url.includes('.jpg')){
            ...
            if(clicked_element.find(".sk-ig-icon.sk-ig-icon--carousel").length > 0) {
                post_items += "<a data-toggle='tooltip' data-placement='top' title='View on Instagram' class='sk-ig-icon sk-ig-icon--carousel sk-ig-icon-popup' target='_blank' href='https://www.instagram.com/p/" + code + "'><i class='fa fa fa-files-o' aria-hidden='true'></i></a>";
            } 
            else if (clicked_element.find(".sk-ig-icon.sk-ig-icon--video").length > 0 && getDsmSetting(sk_instagram_feed, "show_play_icon") == 1) {
                post_items += `<a data-toggle='tooltip' data-placement='top' title='View on Instagram' class='sk-ig-icon sk-ig-icon--video sk-ig-icon-popup' target='_blank' href='https://www.instagram.com/p/${code}'>
            ...
            if (data_type == 'video') {
                post_items+="<img src='" + video_thumbnail_src + "' class='ig_media' style='height:auto;    vertical-align: middle;object-fit: contain;'>";
            } else {
                post_items+="<img src='" + thumbnail_src + "' class='ig_media' style='height:auto;    vertical-align: middle;object-fit: contain;'>";
            }
        }
        ...
        post_items += "<div class='instagram-user-root-container "+is_slider_layout+"'>";
            ...
            if (show_profile_picture == 1) {
                ...
                post_items += "<div class='sk-instagram-profile-pic_container'>";
                    post_items += "<img crossorigin='anonymous' src='"+ updateUrl(sk_instagram_feed, picSrc) +"' alt='sk-inst-profile-picture'>";
                post_items += "</div>";
            }
            post_items += "<div class='sk-ig-profile-info ' " + width + ">";
            post_items += "<div class='sk_ig_feed_username_follow'>";
            if (show_profile_username == 1) {
                post_items += "<a class='href_status_trigger' href='https://www.instagram.com/" + data.bio.username + "/' target='" + openLinkTab() + "'><p class='sk-ig-profile-usename' style='color:rgb(85, 85, 85);'>@" + data.bio.username;
            ...
            if (show_profile_name == 1 || show_profile_description == 1 || show_profile_website == 1 || show_profile_follow_button == 1) {
                    ...
                    if (show_profile_website == 1 && data.bio.website_link) {
                        ...
                        post_items += `
                            <div>
                                <a class="bio-website" href="${data.bio.website_link}" target="${openLinkTab()}">
                        ...
                        post_items += `
                            <div>
                                <a class="bio-website" href="${data.bio.external_url}" target="${openLinkTab()}">
                                <svg xmlns="http://www.w3.org/2000/svg"
                                    fill="none"
                                    width="15"
                                    height="15"
                        ...
                        post_items += `
                            <div>
                                <a class="bio-website" href="${data.bio.website_url}" target="${openLinkTab()}">
                                <svg xmlns="http://www.w3.org/2000/svg"
                                    fill="none"
                                    width="15"
                                    height="15"
                    ...
                    if(show_profile_follow_button==1){
                        post_items+="<button type='button' onclick=\"window.open('https://www.instagram.com/" + data.bio.username + "/');\" class='instagram-user-container'>";
                            post_items+="<span style='font-family: FontAwesome !important; margin-right: 5px;' aria-hidden='true'>&#xf16d;</span>" + follow_text;
                        post_items+="</button>";
                    }
            }
            post_items += "</div>";
...
function getFeedItem(val,sk_instagram_feed, data_position) {
    ...
    post_items += "<div profile-pic-url='"+data_bio[embed_id].profile_pic_url_hd+"' class='"+height_auto_class+" sk-instagram-feed-item ' " + post_title_hover + " data-link='" + val.link + "' data-code = '" + val.code + "' data-type='" + val.pic_type + "' video-url='"+val.video_url+"'  data-position='"+data_position+"' data-username='"+data_bio[embed_id].username+"'><img onerror='handleImageError(this)' data-image='" + val.code + "' class='sk-img-sizer' src='" + val.image_url + "' style='display: none;width:100%;' />";

    if (val.pic_type == "video" && getDsmSetting(sk_instagram_feed, "show_play_icon") == 1) {
        post_items += `<div class='sk-ig-icon sk-ig-icon--video'>
            <svg class='sk-ig-icon__svg' xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" width="24" height="24">
            <path fill-rule="evenodd" d="M4.5 5.653c0-1.427 1.529-2.33 2.779-1.643l11.54 6.347c1.295.712 1.295 2.573 0 3.286L7.28 19.99c-1.25.687-2.779-.217-2.779-1.643V5.653Z" clip-rule="evenodd" />
            </svg>
        </div>`;
    }
    else if (val.pic_type == "carousel" && getDsmSetting(sk_instagram_feed, 'show_carousel_icon') == 1) {
        post_items += "<span class='sk-ig-icon sk-ig-icon--carousel'><i class='fa fa fa-files-o sk-ig-icon__fa' aria-hidden='true' style='font-size:20px; line-height:28px;'></i></span>";
    }

    // show on load
    post_items += "<div class='sk-ig-post-hover display-none'>";
        if(val.pic_like_count_formatted){
            const likes = formatLikes(parseInt(val.pic_like_count_formatted, 10));
            post_items += "<span class='m-r-15px'>";
            post_items += "<i class='fa fa-heart' aria-hidden='true'></i> <span class='sk-like-count'>" + likes + "</span>";
            post_items += "</span>";
        }
        if(val.pic_comment_count_formatted){
            post_items += "<span>";
            post_items += "<i class='fa fa-comment' aria-hidden='true'></i> <span class='sk-comment-count'>" + val.pic_comment_count_formatted + "</span>";
            post_items += "</span>";
        }
    post_items += "</div>";

    post_items += "<div data-image='" + val.code + "' style='background-size:cover; background-position:center; background-image:url(" + val.image_url + Math.floor(Date.now() / 1000) % 1000 + ");' class='sk-ig-post-img'></div>";
    ...
    if (getDsmSetting(sk_instagram_feed, "post_item_type") == 1) {
        post_items += "<div class='sk-ig-caption-container' style='background-color:#fff;'>";
            if (getDsmSetting(sk_instagram_feed, "show_likes_count") == 1 && parseInt(val.pic_like_count_formatted) > 0) {
                ...
                post_items += "<i class='fa fa-heart' aria-hidden='true'></i> " + val.pic_like_count_formatted;
                post_items += "</span>";
            }

            if(getDsmSetting(sk_instagram_feed, "show_ago_value") == 1){
                var now = moment();
                var post_date = moment(val.date_time_posted);
                var hoursDifference = now.diff(post_date, 'hours');
                var daysDifference = now.diff(post_date, 'days');
                if(hoursDifference > 23){
                    post_items += "<span class='sk-ig-feed-m-r-15px sk-ago-data'>";
                        post_items += daysDifference + " " + (daysDifference === 1 ? "day" : "days") + " ago";
                    post_items += "</span>"; 
                } else {
                    post_items += "<span class='sk-ig-feed-m-r-15px sk-ago-data'>";
                        post_items += moment(val.date_time_posted).fromNow();
                    post_items += "</span>";
                }
            }
            ...
            if (getDsmSetting(sk_instagram_feed,'show_ig_icon') == 1) {
                post_items+="<a class='sk-view-on-ig-link float-right' data-code='"+val.code+"' href='" + val.link + "' target='_blank'>";
    ...
    if (val.pic_type == 'video') {
        if(original_data[embed_id].settings 
            ...
            if (getDsmSetting(sk_instagram_feed, "show_play_icon") == 1) {
                post_items += "<div onclick=\"window.open('https://www.instagram.com/p/" + val.code + "/');\" class='sk-ig-icon sk-ig-icon--video'><i class='fa fa-play-circle' aria-hidden='true'></i></div>";
            }
            ...
            post_items += '<div class="swiper-wrapper sk_loading_carousel">';
                val.children.forEach(function (element, index) {
                    if (index === 0 && val.image_url && element.media_type == "IMAGE") {
                        post_items += "<div class='swiper-slide'><img onerror='handleImageError(this)' src='" + val.image_url + "' class='ig_media' style='height:auto; vertical-align: middle;object-fit: contain;'/></div>";
                    } 
                    else if(element.media_type == "IMAGE"){
                        if (val.sk_images[index]) {
                            post_items += "<div class='swiper-slide'><img onerror='handleImageError(this)' src='" + val.sk_images[index] + "' class='ig_media' style='height:auto; vertical-align: middle;object-fit: contain;'/></div>";
                        } 
                    }
        ...
        if(val.pic_type == 'carousel'){
            post_items += "<span onclick=\"window.open('https://www.instagram.com/p/" + val.code + "/');\" class='sk-ig-icon sk-ig-icon--carousel'><i class='fa fa-files-o' aria-hidden='true'></i></span>";
        }
        post_items += "<img onerror='handleImageError(this)' data-image='" + val.code + "' img-url='" + val.image_url + "' src='" + val.image_url + "' class='ig_media sk-image-sizer' />";
    }
    post_items += "</div>";
    if (show_info_container == 1) {
        post_items += "<div class='sk_popup_column' >";
            post_items += "<div class='sk_popup_column_spacer'>";
                ...
                if(getDsmSetting(sk_instagram_feed, "show_popup_profile_picture") == 1
                    ...
                    post_items += "<div class='sk_popup_column_user'>";
                        if (getDsmSetting(sk_instagram_feed, "show_popup_profile_picture") == 1 && original_data[embed_id].bio.user_profile) {
                            post_items += "<img onerror='handleImageError(this)' class='sk-popup-profile-image' src='"+original_data[embed_id].bio.user_profile+"' data-image='"+original_data[embed_id].bio.username+"' />";
                        } else if (getDsmSetting(sk_instagram_feed, "show_popup_profile_picture") == 1 && original_data[embed_id].bio.profile_pic_url) {
                            post_items += "<img onerror='handleImageError(this)' class='sk-popup-profile-image' src='"+original_data[embed_id].bio.profile_pic_url+"' data-image='"+original_data[embed_id].bio.username+"' />";
                        }
                        post_items += "<p>";
                        if (getDsmSetting(sk_instagram_feed, "show_pop_up_username") == 1) {
                            post_items += "<a class='href_status_trigger sk-instagram-feed-username' href='https://www.instagram.com/" + val.owner_username + "/' target='" + openLinkTab() + "'><span><b>" + val.owner_username + "</b></span></a><br>";
                        }
                        if (getDsmSetting(sk_instagram_feed, "show_ago_value") == 1 && val.date_time_posted) {
                            ...
                            var hoursDifference = now.diff(post_date, 'hours');
                            var daysDifference = now.diff(post_date, 'days');
                            var formattedDate = post_date.format("MMM D, YYYY");
                ...
                if (getDsmSetting(sk_instagram_feed, "show_likes_count") == 1 || getDsmSetting(sk_instagram_feed, "show_comments_count") == 1) {
                    ...
                    if (parseInt(val.pic_like_count_formatted) > 0 && getDsmSetting(sk_instagram_feed, "show_likes_count") == 1) {
                        post_items += "<span>";
                        post_items += "<i class='fa fa-heart' aria-hidden='true'></i> <span class='sk-like-count'>" + val.pic_like_count_formatted + "</span>";
                        post_items += "</span>";
                    }
                    val.pic_comment_count_formatted = 25;
                    if (parseInt(val.pic_comment_count_formatted) > 0 && getDsmSetting(sk_instagram_feed, "show_comments_count") == 1 || 1==1) {
                        post_items += "<span>";
                        post_items += "<i class='fa fa-comment' aria-hidden='true'></i> <span class='sk-comment-count'>" + val.pic_comment_count_formatted + "</span>";
                        post_items += "</span>";
                    }
                    ...
                    if (links_clickable == 1) {
                        if (getDsmSetting(sk_instagram_feed, "show_view_on_twitter_link") == 1) {
                            ...
                            post_items += "<a class='href_status_trigger' style='display: inline-table;' href='" + val.link + "' target='" + openLinkTab() + "'>";
...
function applyCustomUi(jQuery, sk_instagram_feed){
    ...
    var pic_width = parseFloat((sk_instagram_post_container_width / column_count) - (space_between_images));
        ...
        html += "<a href='" + response.url + "' link-only target='_blank' style='text-decoration: none;'>";
            html += "<div class='sk-link-article-container' style='background: #eeeeee;color: black !important; font-weight: bold !important; border-radius: 2px; border: 1px solid #c3c3c3; box-sizing: border-box; word-wrap: break-word;'>";
                ...
                if (show_thumbnail == 1 && response.thumbnail_url) {
                    ...
                    html += "<img alt='No alternative text description for this image' "+display_none+" class='sk-link-article-image sk_post_img_link' onerror='this.style.display=\"none\"' src='" + (response.thumbnail_url || attr_thumbnail) + "'/>";
                } else if (attr_thumbnail && attr_thumbnail.length > 0){
                    html += "<image alt='No alternative text description for this image' class='sk-link-article-image sk_post_img_link' onerror='this.style.display=\"none\"' src='" + attr_thumbnail + "'/>";
                }
...
function skGetBranding(sk_, user_info) {
    ...
    // remove email from settings - for freee plan user
    ...
    // generate tutorail link
    if(user_info.created_at_ymd && user_info.created_at_ymd > 20240725){
        user_info.tutorial_link = getTutorialLinkNew(user_info);
    }
    else{
        user_info.tutorial_link = getTutorialLink(user_info);
    }
    ...
    if (user_info.show_branding && (user_info.show_branding == 1
        ...
        html += "<div class='sk_branding' style='padding:10px; display:block !important; text-align:center; text-decoration: none !important; color:#555; font-family:" + fontFamily + "; font-size:15px;'>";
            html += "<a "+nofollow_attribute+" class='tutorial_link' href='"+user_info.tutorial_link+"' target='_blank' style='text-underline-position:under; color:" + link_color + ";font-size:15px;'>";
